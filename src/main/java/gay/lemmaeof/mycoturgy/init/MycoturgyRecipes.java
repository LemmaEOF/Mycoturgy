...
public class MycoturgyRecipes {
	...
	public static <T extends Recipe<?>> RecipeType<T> register(String name) {
		return Registry.register(Registries.RECIPE_TYPE, new Identifier(Mycoturgy.MODID, name), new RecipeType<T>() {
			@Override
	...
	public static <T extends Recipe<?>> RecipeSerializer<T> register(String name, RecipeSerializer<T> serializer) {
		return Registry.register(Registries.RECIPE_SERIALIZER, new Identifier(Mycoturgy.MODID, name), serializer);
	}
}
